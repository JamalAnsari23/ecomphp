...
function add_row()
{
 ...
 $rowno=$rowno+1;
 $("#employee_table tr:last").after("<tr id='row"+$rowno+"'><td><input type='text' name='description[]' placeholder='Title'></td><td><input type='text' name='description[]' placeholder='Heading'></td><td><input type='text' name='description[]' placeholder='Description'></td><td><input type='button' value='DELETE' onclick=delete_row('row"+$rowno+"')></td></tr>");
}
...
function add_pdf()
{
 ...
 $rownox=$rownox+1;
 $("#product_pdf tr:last").after("<tr id='rowpdf"+$rownox+"'><td><input type='text' name='name[]' placeholder='PDF Heading'></td></td><td><input type='file' name='name[]' placeholder='PDF'></td><td><input type='button' value='Deletepdf' onclick=delete_rowx('rowpdf"+$rownox+"')></td></tr>");
}
...
<div id="wrap">
  ...
  <div class="form-group">
   ...
   <tr id="row1">
    <td><input type="text" name="description[]" placeholder="Title"></td>
    <td><input type="text" name="description[]" placeholder="Heading"></td>
  ...
  <div class="form-group">
   ...
   <tr id="rowpdf1">
    <td><input type="text" name="name[]" placeholder="PDF Heading"></td>
    <td><input type="file" name="name[]" class="form-control-file" id="exampleFormControlFile1"></td>
   </tr>
  </table>
  ...
  <div class="form-group form-check">
			...
			<label style="margin-right: 95px;">Active</label>
...

{
   ...
   $short_description = $_POST["short_description"];
            ...
            $description1 = implode(",",$lang);    //that is use for array convert into strings//
   $features = $_POST["features"];
   $filename2 = $_FILES["name"]["name"];
			$tempname2 = $_FILES["name"]["tmp_name"];
			$folder2 = "images2/".$filename2;
			move_uploaded_file($tempname2, $folder2);
   $pdf_description   = $_POST['name'];
   $pdf_description1 = implode(",",$lang); 
   $status = $_POST['radio'];

   $sql = "INSERT INTO `products` (`cat_name`,`sub_cat_name`,`product_name`,`product_image`,`short_description`,`description`,`features`,`pdf_description`,`status`)
   VALUES('$cat_name','$sub_cat_name','$product_name','$filename','$short_description','$description1','$features','$pdf_description','','$status')";
...
if($total != 0) 
{
  ...
  while($result = mysqli_fetch_assoc($data))
  {
    ...
    <td>".$result['product_id']."</td>
        ...
        <td>
		<a href='updateform_data.php?id=$result[product_id]'><input type = 'submit' value = 'Update' class ='update'></a>
